...
|  | gamebase-adapter-auth-hangame | hangame-id-1.6.1 | Support Hangame login | - |
...
| Gamebase IAP Adapters | gamebase-adapter-toastiap | toast-gamebase-iap-0.21.0<br>nhncloud-iap-core | Support in-app purchase | - |
...
* For payment test, register as a tester as follows for each store: (This is not settings for Gamebase Tester registration but for test payment of the store.)
    ...
    * ONE store
        ...
        * `Currently not supported` [ONE store > Document > Development Tool > Introduction and Download for ONE store In-App Purchase API V7 (SDK V21) > 03. Purchase Test and Security > Register/Manage Test ID](https://dev.onestore.co.kr/wiki/ko/doc/03-37552542.html#id-03.결제테스트및보안-테스트ID등록/관리)
    * GALAXY Store
