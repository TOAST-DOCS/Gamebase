...
2. The game server checks the game DB to see if there is a history of already providing an item with the same paymentSeq.
    * 2-1. If the item has not been provided yet, call the Gamebase server's Payment Transaction API to verify that the paymentSeq value and is matched with the response field and purchaseToken value is valid.
...
* See the guide for the NHN Cloud IAP error codes.  
    * [NHN Cloud > User Guide for NHN CLoud SDK > NHN Cloud IAP > iOS > Error Codes](https://docs.nhncloud.com/en/nhncloud-sdk/en/iap-ios/#error-codes)
