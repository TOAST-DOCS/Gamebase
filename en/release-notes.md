...
#### Feature Updates
* [SDK] 2.20.2
	* (Android) Updated to Billing Client Version 3.0.3 where payment errors caused by Android 11 devices in Google Play Store are fixed

### March 23, 2021 

#### Feature Updates
* [Console] Member > Download: Increased the size of data stored in a file (from 50,000 to > 500,000)
* [SDK] 2.20.2
	* (iOS) Facebook iOS SDK updated (9.1.0)
	* (iOS) Fixed an issue of failing to call openURL delegate from GamebaseAuthFacebookAdapter in certain cases
...
* Common Terms and Conditions added
	* [Console] New menus available: App > Terms and Conditions, App > Deploy Terms and Conditions
	* [SDK] 2.20.0
		* (Common) Added an API that opens the Terms and Conditions webview
		* (Common) Added an API that views the Terms and Conditions list and agreement status per user
		* (Common) Added an API that saves the user agreement data on the Gamebase server

#### Feature Updates
* [Console] App > Client: Improved the screen to classify and display the client version per store
* [SDK] 2.20.0
	* (Common) Changed to display the Customer Center without login if the Customer Center type is TOAST organization product (Online Contact).
	* (Unity) Removed warning logs
	* (Unity) Updated CEF 2.1.2 for Standalone WebView
...
#### Bug Fixes
* [SDK] 2.20.0
	* (JavaScript) Fixed an error that occurs during initialization when console has no Customer Center information
* [SDK] 2.19.1
	* (Unreal) Fixed a compile error caused by files excluded during Unity Build
