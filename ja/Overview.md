...
ゲーム会社は開発したゲームを複数のストアにリリースすることで、小さい労力で収益を最大化することができます。Gamebaseは簡単に複数のストアの連携をサポートするため、主要ストアごとに決済連携仕様を学ぶ必要がありません。
...
* **複数のストアのアプリ内決済を単一インターフェイスで提供します。**
  単一インターフェイスでAPIを提供し、より簡単かつ迅速にストアの追加が可能になるため、開発費用を抑えることができます。開発者は複雑な決済連携方法を学ばなくても簡単に決済機能を実装できます。
...
* **単一アイテムの購入だけでなく、定期購入、Promotion機能を提供します。**
  GoogleストアとAppStoreで提供する定期購入機能を提供し、月額商品をユーザーに販売できます。ゲームでは別途実装せずに簡単にGoogleのプロモーション機能も使用できます。外部ストアで追加される機能は今後も継続してGamebaseにも追加機能として提供する予定です。
* **Webコンソールでの多様な機能(決済履歴照会機能など)で、顧客からの問い合わせに円滑に対応できます。**
  Webコンソールでユーザーの決済履歴とアイテム支給状態の確認ができます。決済のキャンセルおよび不正行為対応も可能です。
