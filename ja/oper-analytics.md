...
LTVは選択された利用者グループで利用者1人の1年間の期待売上を表す推定指標です。

国/OS別、日時別にLTVチャートを提供し、下段の表にはLTV、累積NRU、累積PU、累積決済金額情報を詳細に確認できます。

#### 推定方法
GamebaseではLTVの推定方法として、登録後365日経過時点の累積ARPUを使用します。 
...
#### 制限条件
LTVを正確に推定するために、次の制限条件があります。

* 利用者グループの利用者数が1000人以上いる必要があります。
* 利用者グループのPU(決済利用者数)が30以上いる必要があります。
* 利用者グループの中で、最も新しい加入日から7日が経過している必要があります。
...
Life Cycleは初めて利用者が流入した時点から、日別の利用者推移を確認できる指標です。データは最大3年まで提供されます。

* 1日の利用者(DAU)：1日あたりのmemberno基準でログイン1回以上のアクティブ利用者数(Daily Active Users)
* 最大同時接続者(MCU)：0時～24時の最大同時接続者数。1分単位CCU値から最も大きい値を1日単位で集計する。
* 新規加入者(NRU)：新規加入者。当日0時～24時までログインログが初めて収集された利用者(memberno基準)
* 退会利用者：退会した利用者。当日0時～24時までmembernoが削除された利用者
* 加入当日退会利用者：当日に加入して、当日に退会した利用者
* 平均CCU：選択された期間内のCCUの平均
...
Frequency7指標はDAUの1週間の訪問数と比率情報を提供します。ゲーム没入度、忠誠度などをひと目で把握できます。

Frequency7の基準は次の3つに分けられます。

* 訪問回数：7日間の合計訪問回数
* 連続訪問回数：7日間で該当日を含む連続訪問回数
* 最大連続訪問回数：7日間で連続訪問した最大回数

上の3つの基準の計算方法例は次の通りです。 
3/7を基準に3/1, 3/2, 3/3, 3/6, 3/7に訪問したユーザーがいる場合、基準に基づく訪問回数は次の通りです。

* 合計訪問回数：5日(3/1, 3/2, 3/3, 3/6, 3/7)
* 連続訪問回数：2日(3/6, 3/7)
...
> [参考] 
>
> ワールド/サーバー/チャンネル、クラス/職業は事前に登録された情報のみ処理します。
> 登録方法は次の文書を参照してください。
>
