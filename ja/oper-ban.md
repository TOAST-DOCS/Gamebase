...
- **連携有無**：AppGuardで検知または制裁したユーザーを、自動的にGamebase利用停止利用者として登録したい場合に有効にします。
- **永久利用停止**を行いたい検知/制裁の自動利用停止を**即時遮断**を選択して「ユーザー表示メッセージ」と**利用停止期間**を入力して**保存**ボタンを押すと適用されます。
- **条件利用停止**を行いたい検知/制裁の自動利用停止を**条件遮断**を選択して検知期間と検知回数を入力して**保存**ボタンを押すと適用されます。
  - 条件遮断による利用停止は、1日1回0時に実行されます。
  - 改ざんの検知期間：5、検知回数：3と設定した場合
    - 01/19日0時に01/14～01/18の間、特定ユーザーが改ざんによる検知が3回以上検出された場合、「利用停止」と録されます。
- **リーダーボード連携**項目を選択すると、検知または制裁されたユーザーのリーダーボードデータを削除するかどうかを選択できます。
