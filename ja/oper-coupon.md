...
現在設定できる最大数は99個で、0に設定すると無制限で使用可能に設定できます。
...
##### (2)テンプレート選択：Toast Cloud SMSサービスでテンプレートを登録している場合は、テンプレートを選択して送信できます。登録されたテンプレートがない場合は選択する必要がありません。

##### (3)広告性かどうか：送信するSMSが広告性かどうかを選択します。広告性で送信する場合(広告)、[無料受信拒否]文字列が含まれている場合のみ、SMSを送信できる点に注意してください。

##### (4)送信番号：Toast Cloud SMSに登録した送信番号を選択できます。この番号は受信者に送信者番号として表示されます。
...
##### (8)受信拒否番号：Toast Cloud SMSに登録されている受信拒否番号を選択します。広告性SMSを送信する場合、この項目を内容に入力する必要があります。送信画面での選択項目は参考用に表示するために選択される値のため、送信時に注意してください。
...
#### 7. クーポン統計
クーポン発行詳細情報画面の下部でSMS送信履歴を照会することができます。送信に関連する統計の照会とファイルのダウンロードを行うことができます。
