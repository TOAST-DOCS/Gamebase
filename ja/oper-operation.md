...
現在進行中の告知リストから直接行を動かして表示順序を設定することができ、ゲームで表示APIを呼び出すと、上部に登録されたイメージが優先的に表示されます。同じ期間に最大5個の告知イメージを表示できます。
...
#### (5)イメージタップ動作
ゲームユーザーがイメージ告知を押した時に処理する動作を設定します。
可能な設定は以下の通りです。

- **URLを開く**：移動するURLを入力します。押した後、入力したURLを新しいWebビューで開き、イメージ告知ウィンドウは自動的に閉じられません。
- **Payload**：入力したデータをクライアントに渡します。渡された値を利用して、イメージを押した時にゲーム内画面に移動したり、特定のイベント処理を行えます。押した後、イメージ告知は全て終了します。
- **動作なし**：イメージ告知を押しても何も動作をしません。

> [参考]
>
> **URLを開く**項目に特定**http**URLを使用したい場合には、Androidビルドにドメイン例外宣言を追加する必要があります。
> そうしない場合、OSの制約によりAndroid 9.0以上の端末では正常にページが表示されません。


#### (6)イメージ
ゲーム内に表示するイメージを登録します。
言語ごとに表示したいイメージを設定することができ、端末の言語に合わせてイメージが表示されます。
登録可能なファイル形式はJEPG、JPG、PNGで、サイズは最大10MBまでです。
...
#### (7)ポップアップ色テーマ
イメージ告知表示時のWebビューテーマを設定できます。
**ダークモード**と**ライトモード**のどちらかのテーマを選択して保存することができ、デフォルトで**ダークモード**が指定されています。
...
この入力情報は、ゲームユーザーに表示されず、このキックアウトを登録する簡単な理由について入力してください。

### (3)メッセージ
ユーザーに表示するキックアウトメッセージです。
