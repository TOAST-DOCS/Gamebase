...
* APIの使用方法を変更しました。
    ...
    * **UGamebaseSubsytem**はGameInstanceのサブシステムであるため、GameInstanceのライフサイクルに従い、SDK API呼び出し時に使用するGameInstanceを通じて該当サブシステムを探してAPIを使用する必要があります。
* GamebaseInterfaceモジュールが削除されました。Gamebaseプラグインを使用する場合、GamebaseInterfaceモジュールを削除してから使用してください。
