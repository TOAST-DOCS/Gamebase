...
|         | -4000921 ~ 2          | TransferAccount 발급시 내부 오류 발생. 계속 발생시 고객센터를 통해 문의 필요 |
|         | -4000923              | 메뉴얼(MANUAL) 방식으로 PASSWORD 를 변경하려고 할 때, 현재 PASSWORD와 동일한 문자열 입력 |
|         | -4000924              | 내부 오류. 계속 발생시 고객센터를 통해 문의 필요 |
|         | -4000925              | 메뉴얼(MANUAL) 방식으로 ID 를 변경하려고 할 때, 현재 사용중인 ID를 입력 |
|         | -4000927              | TransferAccount 유효성 검증시, PASSWORD 가 잘못 되었을 때 |
|         | -4040920              | TransferAccount 유효성 검증시, ID 가 존재하지 않을 때 |
|         | -5110920              | TransferAccount 발급 시스템 오류. 계속 발생시 고객센터를 통해 문의 필요 |
| Coupon  | -100002 ~ 3           | 잘못된 쿠폰 코드를 요청한 경우                   |
|         | -100004               | 요청에 의해 수동으로 이미 만료 처리한 쿠폰 코드    |
|         | -100005               | 이미 사용된 쿠폰                                |
|         | -100006 ~ 10          | 쿠폰 사용 기간이 아닌 경우                       |
|         | -100011               | 사용 가능한 쿠폰 제한 수량을 초과한 경우          |
|         | -999999               | 쿠폰 시스템 내부 오류. 계속 발생시 고객센터를 통해 문의 필요  |
