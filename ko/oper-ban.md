...
- **상태**
  - <font color="white" style="background-color:#FB8F37">이용정지</font>: 게임 유저가 현재 앱에 접속할 수 없는 상태
  - <font color="white" style="background-color:#A1A1A1">이용정지(기간 만료)</font>: 게임 유저의 이용 정지 기간이 끝났으나 현재 로그인을 하지 않은 상태. 게임 유저가 재접속 시 해제(기간 만료)로 상태가 변경됨
...
- **영구 이용 정지**를 원하는 탐지/제재 종류의 자동 이용 정지를 **즉시 차단**을 선택하여 '사용자 노출 메시지'와 **이용 정지 기간**을 입력하고 **저장** 버튼을 클릭하면 적용됩니다.
- **조건 이용 정지**를 원하는 탐지/제재 종류의 자동 이용 정지를 **조건 차단**을 선택하여 탐지 기간과 탐지 횟수를 입력하고 **저장** 버튼을 클릭하면 적용됩니다.
  - 조건 차단에 의한 이용정지는 하루에 한 번 자정에 수행됩니다.
  - 변조에 대해 탐지 기간: 5, 탐지 횟수: 3 을 설정한 경우
    - 01/19일 자정에, 01/14 ~ 01/18일 동안 특정 유저가 변조에 의한 탐지가 3회 이상 검출된 경우, 이용정지가 등록됩니다.
- **리더보드 연동** 항목을 선택하면 탐지 또는 제재된 사용자의 리더보드 데이터 삭제 여부를 함께 선택할 수 있습니다.
