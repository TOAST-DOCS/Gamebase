...
- **반복 발송**: 동일한 푸시를 주기적으로 발송합니다. 사용자의 단말기 국가 시간 기준으로 푸시 메시지를 발송하고자 하는 경우 **현지 시간 기준으로 발송**을 선택합니다. 발송하고자 하는 기간을 선택한 후 해당 기간동안 발송하고자 하는 시각을 선택합니다. 그 후 선택한 발송기간동안 발송할 주기를 **매일**, **매주**, **매월** 기준으로 정보를 입력합니다.
	- 매일 : 설정한 푸시를 매일 발송합니다.
	- 매주 : 발송하고자 하는 요일을 선택하여 특정 요일에만 푸시를 발송합니다.
...
- **태그 발송**: 태그를 이용하여 푸시를 발송할 수 있습니다. **푸시 - 태그** 메뉴에서 등록된 태그로 발송이 가능하며 여러개의 태그 등록도 가능합니다. 태그 필터는 아래와 같은 기능을 지원합니다.
	...
	- OR : 선택한 태그 중 하나라도 조건을 만족하는 유저에게 모두 푸시를 발송합니다.
...
#### (4) 알림음(선택항목)
푸시가 수신될 때의 알람음을 설정할 수 있습니다.
외부 URL링크 또는 앱 내 설치된 알림음 파일경로를 입력합니다.
추가를 하고자 할 경우 **알림음 추가**버튼을 누르면 추가할 수 있습니다.
...
Android의 경우에는 푸시 수신시의 글자색을 지정할 수 있는 기능을 제공합니다.
제목/내용의 색을 각각 지정할 수 있으며 글자색 선택창에서 제공되지 않는 항목이라도 RGB Hex값을 직접 입력하여 색을 선택할 수 있습니다.
선택된 글자색의 적용여부는 오른쪽의 미리보기 화면을 통해 확인이 가능합니다.

#### (8) 큰 아이콘(Only Android)(선택항목)
Android 푸시 수신시에 오른쪽에 표시 될 아이콘을 설정할 수 있습니다.
외부/내부를 선택 후 항목값을 입력할 수 있습니다.
외부를 선택했을 경우 표시할 아이콘의 외부 URL을 입력하며 오른쪽 미리보기에서 해당 아이콘을 미리 확인할 수 있습니다.
내부를 선택했을 경우 앱 내의 아이콘 파일경로를 입력합니다.
추가를 하고자 할 경우 **큰 아이콘 추가**버튼을 누르면 추가할 수 있습니다.

#### (9) 버튼
푸시 수신시의 버튼을 유저가 원하는대로 설정하여 함께 보낼 수 있습니다.
버튼은 최대 3개까지 등록 가능하며 각 버튼은 아래 유형과 같이 등록할 수 있습니다.
- 앱 열기 : 버튼을 선택할 경우 해당 앱을 실행합니다. 버튼 이름만 지정 가능합니다.
- URL 열기 : 버튼을 선택할 경우 연결될 링크를 연결합니다. 앱 스키마를 통한 특정 앱 실행 또는 연결된 URL 실행이 가능합니다.
- 응답 : 버튼을 선택할 경우 응답을 할 수 있는 창이 실행됩니다. iOS의 경우 버튼이름을 함께 설정할 수 있습니다.
- 닫기 : 버튼을 선택할 경우 푸시가 닫힙니다. 버튼 이름만 지정 가능합니다.
