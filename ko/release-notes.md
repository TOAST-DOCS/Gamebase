...
#### 기능 추가
* [Console]
	* 멤버 > 다운로드 신규 메뉴 오픈: 가입일, 마지막 로그인 시간 기준으로 게임 유저리스트를 조회하고 파일로 다운로드 할 수 있음
...
* [SDK] 2.4.4
	* (공통)회원 에러 코드 포맷 변경
	* (Unity)GamebaseServerPushType에 Key 추가(TRANSFER_KICKOUT)
* Setting Tool
	* 폴더 구조 변경: `기존 SettingTool을 완전히 삭제 후 재설치하여야 합니다.`
	* 다국어 지원 추가
