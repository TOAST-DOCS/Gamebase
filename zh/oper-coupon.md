...
**发放关键词优惠券时**输入用户可以使用的固有优惠券代码。

#### 4. 商店

选择可使用发放的优惠券的商店。
默认选项已设置为”所有商店”。若需要按商店类型限制优惠券的使用时，则指定要使用优惠券的商店。
...
#### 2. 商店
选择可使用发放的优惠券的商店。
发放时选择的商店已被设置为默认选项。若修改信息，只可以在更改的商店使用发放的优惠券。
...
#### 6. 优惠券发布
使用发放的优惠券信息，提供可向用户直接发送优惠券的功能。

![gamebase_ban_01_201812](https://static.toastoven.net/prod_gamebase/gamebase_coupon_04_202003.png)
##### 1) 讯息类型 : 选择讯息类型（支持MMS/SMS）。您选择MMS时应添加标题后发送。 
##### 2) 模板 : 如果在Toast Cloud SMS服务中已注册模板，则可选择该模板后发送。若没有注册模板时则不需要选择。
##### 3) 广告类型 : 发送SMS时选择是否发送广告型SMS。如果要发送广告型SMS，只有包含[免费拒绝接收]字符串才能发送。
##### 4) 发送号码 : 可以选择在Toast Cloud SMS中注册的发送号码。接收人将以该号码接收消息。
##### 5) 是否发送 : 选择是否”立即/预约”发送。如果选择立即，点击发送按键则可立即发送。选择预约时，过一定的预约时间后发送。
##### 6) 标题 : 发送MMS时需要输入MMS标题。仅在发送MMS时输入。 
##### 7) 身体 : 输入想要发送的详细内容。字符串是‘’##code##字符串”被转为优惠券的字符串，若不包括该字符串，则不能发送。 
##### 8) 投诉电话 : 选择在Toast Cloud SMS中注册的投诉电话号码。若要发送广告型SMS，应将其内容在身体中输入。发送页面上的选项是为了参考而选择的值，因此发送时要注意。 
##### 9) 接收号码 : 通过上传”接收对象文件”进行注册。可以以**号码**或**号码/优惠券**的形式注册。如果只注册号码，则按号码顺序发送。
##### 10) 发送测试 : 可以按输入的形式发送测试SMS。输入接收号码后点击发送按键，则可向输入的号码发送测试SMS。发送之前，最好先通过发送测试SMS确认形式后发送。
...
#### 7. 发行额外的优惠券
优惠券类型为序列号时，每一次可以发放10万个，最多100个(包括一开始发放的个数)额外的优惠券。
...
通过申请的发送记录可以确认SMS发送记录，并通过点击右边的”下载按键”，下载”发送结果详细明细”。
请求下载后的状态如下。
- **准备创建** : 已完成下载请求。
- **正在创建** : 正在创建请求下载的文件。
- **下载** : 成功创建下载文件后，“下载”按钮被激活，当您单击该按钮时，可以下载“发送详细明细”。
